...
#[cfg(test)]
mod tests {
    use super::{BufferEvent, EditorEvent, InputEvent, SystemEvent, UIEvent};
    ...
    fn test_event_macros() {
        ...
        let input_event = event!(input::key_press(key_event));
        let buffer_event = event!(buffer::modified(1));
        let ui_event = event!(ui::redraw);
        let system_event = event!(system::quit);
