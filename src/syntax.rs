...
mod tests {
    ...
    fn test_semantic_categories() {
        // Test that semantic categories convert to strings correctly
        ...
        assert_eq!(SemanticCategory::Field.as_str(), "field");
        assert_eq!(SemanticCategory::Identifier.as_str(), "identifier");
    }
    ...
    fn test_language_support() {
        ...
        assert!(!rust_support.node_mappings.is_empty());

        // Check some key mappings
        assert_eq!(
            rust_support.node_mappings.get("fn"),
            Some(&SemanticCategory::Keyword)
        );
        assert_eq!(
            rust_support.node_mappings.get("string_literal"),
            Some(&SemanticCategory::String)
        );
        assert_eq!(
            rust_support.node_mappings.get("field_identifier"),
            Some(&SemanticCategory::Field)
        );
        assert_eq!(
            rust_support.node_mappings.get("type_identifier"),
            Some(&SemanticCategory::Type)
        );
    }
    ...
    fn test_language_detection() {
        ...
        assert_eq!(
            highlighter.detect_language_from_extension("test.rs"),
            Some("rust".to_string())
        );
        // Note: test.txt might return Some("text") from editor config fallback
        let txt_result = highlighter.detect_language_from_extension("test.txt");
        assert!(txt_result.is_none() || txt_result == Some("text".to_string()));

        // Test content detection
        let rust_content = "fn main() { println!(\"Hello\"); }";
        assert_eq!(
            highlighter.detect_language_from_content(rust_content),
            Some("rust".to_string())
        );

        // Test combined detection
        assert_eq!(highlighter.detect_language(Some("test.rs"), ""), "rust");
        assert_eq!(highlighter.detect_language(None, rust_content), "rust");
        // Combined detection with unknown extension falls back to content or default
        let unknown_result = highlighter.detect_language(Some("test.txt"), "unknown content");
        assert!(unknown_result == "text" || unknown_result == "rust" || !unknown_result.is_empty());
    }
    ...
    fn test_comment_highlighting() {
        ...
        let test_code = r#"// This is a comment
fn test() {
    /* Block comment */
    let x = 5;
}"#;
        ...
        println!("Comment highlighting test:");
        for highlight in &highlights {
            let text = &test_code[highlight.start..highlight.end];
            println!("  '{}' at {}..{}", text, highlight.start, highlight.end);
        }

        // Check that comments are highlighted
        ...
        assert!(!comment_highlights.is_empty(), "Should highlight comments");
    }

    #[test]
    fn test_struct_highlighting() {
        let mut highlighter = SyntaxHighlighter::new().unwrap();

        let test_code = "struct Point { x: f64, y: f64 }";
        let highlights = highlighter.highlight_text(test_code, "rust").unwrap();

        // Print highlights for debugging
        println!("Struct highlighting test:");
        for highlight in &highlights {
            let text = &test_code[highlight.start..highlight.end];
            println!("  '{}' at {}..{}", text, highlight.start, highlight.end);
        }
        ...
        assert!(!highlights.is_empty(), "Should have highlights for struct");

        // Should highlight the struct keyword
        let struct_highlights: Vec<_> = highlights
            .iter()
            .filter(|h| &test_code[h.start..h.end] == "struct")
            .collect();
        assert_eq!(
            struct_highlights.len(),
            1,
            "Should highlight 'struct' keyword"
        );
    }
