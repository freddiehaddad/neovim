...
use crate::syntax::{HighlightRange, SyntaxHighlighter, HighlightCacheKey, HighlightCacheEntry};
...
impl AsyncSyntaxHighlighter {
    ...
    pub fn get_cached_highlights(&self, buffer_id: usize, line_index: usize, content: &str, language: &str) -> Option<Vec<HighlightRange>> {
    ...
    pub fn request_highlighting(
        ...
        self.request_tx.send(request)
            .map_err(|_| anyhow::anyhow!("Failed to send highlight request - worker may be shut down"))?;
        
        debug!("Requested highlighting for buffer {} line {} with priority {:?}", buffer_id, line_index, priority);
        Ok(response_rx)
    }
    ...
    pub fn request_batch_highlighting(
        ...
        for (line_index, content) in lines {
            let receiver = self.request_highlighting(buffer_id, line_index, content, language.clone(), priority)?;
    ...
    pub fn invalidate_buffer_cache(&self, buffer_id: usize) {
        ...
        if let Ok(mut cache) = self.shared_cache.write() {
            ...
            cache.clear();
            debug!("Invalidated cache for buffer {} (cleared {} entries)", buffer_id, before_size);
        }
    }
    ...
    async fn worker_loop(
        ...
        while let Some(request) = request_rx.recv().await {
            ...
            while let Some(request) = pending_requests.pop() {
                ...
                if pending_requests.len() < 10 { // Don't interrupt if we have a big backlog
    ...
    async fn process_request(
        ...
        if let Ok(cache_ref) = cache.read() {
            if let Some(entry) = cache_ref.get(&cache_key) {
                debug!("Worker cache hit for buffer {} line {}", request.buffer_id, request.line_index);
        ...
        let highlight_ranges: Vec<HighlightRange> = highlights.into_iter().map(|(start, end, highlight_type)| {
            HighlightRange {
                start,
                end,
        ...
        debug!("Worker computed highlights for buffer {} line {} ({} ranges)", 
               request.buffer_id, request.line_index, highlight_ranges.len());
